...
with open('error_ford.plot', 'w') as ford:
	...
	print('set boxwidth 5.0 absolute', file=ford)
	print("set ylabel 'Tiempo (segundos)'", file=ford)
	print("set xlabel 'Número de nodos'",file=ford)
	print('g(x) = a*x**3', file=ford)
	print("fit g(x) 'error_ford.csv' u 1:4 via a", file=ford)
	print("plot 'error_ford.csv' u 1:3:2:6:5 w candlesticks lt 3 lw 2 notitle whiskerbars, '' u 1:4:4:4:4 w candlesticks lt -1 lw 2 notitle, g(x) lc 7 lw 2 notitle", file=ford)

with open('error_floy.plot', 'w') as floy:
	print('set term eps', file=floy)
	print('set output "error_floy.epsw"', file=floy)
	print('set boxwidth 5.0 absolute', file=floy)
	print("set ylabel 'Tiempo (segundos)'", file=floy)
	print("set xlabel 'Número de nodos'",file=floy)
	print('g(x) = a*x**3', file=floy)
	print("fit g(x) 'error_floy.csv' u 1:4 via a", file=floy)
	print("plot 'error_floy.csv' u 1:3:2:6:5 w candlesticks lt 3 lw 2 notitle whiskerbars, '' u 1:4:4:4:4 w candlesticks lt -1 lw 2 notitle, g(x) lc 7 lw 2 notitle", file=floy)